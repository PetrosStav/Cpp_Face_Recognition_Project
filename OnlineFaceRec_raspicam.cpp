...
RaspiCamCvCapture* camera = 0;	// The camera device.
...
IplImage* getCameraFrame(void)
{
	...
	if (!camera) {
		...
		camera = RaspiCamCvCapture( 0 );
		if (!camera) {
			printf("ERROR in getCameraFrame(): Couldn't access the camera.\n");
			exit(1);
		}
		// Try to set the camera resolution
		raspiCamCvCreateCameraCapture2( camera, CV_CAP_PROP_FRAME_WIDTH, 320 );
		raspiCamCvCreateCameraCapture2( camera, CV_CAP_PROP_FRAME_HEIGHT, 240 );
		// Wait a little, so that the camera can auto-adjust itself
		#if defined WIN32 || defined _WIN32
			Sleep(1000);	// (in milliseconds)
		#endif
		frame = raspiCamCvQueryFrame( camera );	// get the first frame, to make sure the camera is initialized.
		if (frame) {
			printf("Got a camera using a resolution of %dx%d.\n", (int)raspiCamCvGetCaptureProperty( camera, CV_CAP_PROP_FRAME_WIDTH), (int)raspiCamCvGetCaptureProperty( camera, CV_CAP_PROP_FRAME_HEIGHT) );
		}
	}

	frame = raspiCamCvQueryFrame( camera );
	if (!frame) {
...
void recognizeFromCam(void)
{
	...
	while (1)
	{
		...
		//Check how many times it passed and stop at 100 saved train photos
		if (f_times){
...
// MAYBE GET IT TO ONLY TRAIN SOME IMAGES ?
		...
		if (faceRect.width > 0) {	// Check if a face was detected.
			...
			if (nEigens > 0 && confidence > 0.942) {	// Check if the face recognition database is loaded and a person was recognized.
